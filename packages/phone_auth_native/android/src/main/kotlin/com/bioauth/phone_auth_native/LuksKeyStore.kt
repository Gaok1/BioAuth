...
/**
 * The key that wraps LUKS disk keys.
 *
 ...
 * device left unlocked on a table does not unlock a stack of volumes, and it is
 * invalidated when biometrics are re-enrolled.
 *
 * The desktop stores what this produces and cannot read it. Nothing here logs
 * a key, a blob, or a volume binding.
 */
internal class LuksKeyStore(
    ...
    /** Removes the key, and with it every volume only this phone could open. */
    fun deleteKey() {
        keyStore.deleteEntry(KEY_ALIAS)
    }
     ...
     * class's business: to the desktop it is opaque bytes it stores and hands
     * back, which is why the volume format says nothing about it.
     */
    fun wrap(cipher: Cipher, binding: ByteArray, credentialId: String, dataKey: ByteArray): ByteArray {
        require(dataKey.size == DATA_KEY_BYTES) { "A LUKS disk key is 32 bytes" }
    ...
    private fun secretKey(): SecretKey =
        keyStore.getKey(KEY_ALIAS, null) as? SecretKey
            ?: throw IllegalStateException("LUKS wrapping key does not exist")

    private fun ivOf(blob: ByteArray): ByteArray {
        require(blob.size > 2) { "Malformed LUKS wrapper" }
        require(blob[0] == BLOB_VERSION) { "Unsupported LUKS wrapper version" }
        val ivLength = blob[1].toInt() and 0xff
        require(ivLength in 1..MAX_IV_BYTES && blob.size > 2 + ivLength + TAG_BITS / 8) {
            "Malformed LUKS wrapper"
        }
    ...
    companion object {
        /**
         * The additional data a LUKS wrapper is bound to, mirroring
         * `phone_auth_protocol::luks`.
         *
         * Computed here from the binding the desktop sent and the credential id
         * this phone holds, so an edited wrapper id in a volume produces a
         * different value and the tag fails.
         */
        fun wrapperAad(binding: ByteArray, credentialId: String): ByteArray {
            require(binding.size == BINDING_BYTES) { "A volume binding is 32 bytes" }
        ...
        /** Alias for the LUKS wrapping key. Never reused by another purpose. */
        const val KEY_ALIAS = "bioauth_luks_wrapping_v1"
        ...
        private val WRAPPER_DOMAIN = "bioauth-luks-wrapper-v1".toByteArray(Charsets.UTF_8)
    }
}
