...
| FND-08 | P0 | 🧪 | Passkeys carregam o mesmo request ID browser→host→agent→Android. Abort, timeout de browser, timeout do agent e desconexão enviam/cumprem cancelamento; notificação e `BiometricPrompt` são removidos, conclusão é once-only e testes cobrem bridge, registry Rust, frame Dart e coordinator Kotlin. A corrida de criação foi fechada: cancel e commit disputam o mesmo `pending.remove`; se cancel vence, a closure que cria chave/metadata não roda, e se commit vence o cancel tardio não reclassifica a passkey já criada. Dois testes forçam as duas ordens. Idempotência específica de vault/locker ainda impede ✅ global. |
