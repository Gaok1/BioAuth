...
| FND-07 | P0 | ✅ | Pareamentos e passkeys usam envelopes v2 com migração, snapshot anterior, rollback reportado e recusa de versão futura. Locker e cofre nasceram em v1 estrito, sem formato legado a migrar, e publicam atomicamente. O cofre versiona tanto o ciphertext autenticado quanto o JSON cifrado; versão futura em qualquer camada retorna `store_version_unsupported` em vez de corrupção, portanto a recuperação não oferece destruir dados escritos por build mais novo. Testes cobrem round-trip, adulteração e as duas fronteiras de versão futura. |
| FND-08 | P0 | ✅ | Passkeys carregam o mesmo request ID browser→host→agent→Android. Abort, timeout de browser, timeout do agent e desconexão enviam/cumprem cancelamento; notificação e `BiometricPrompt` são removidos, conclusão é once-only e testes cobrem bridge, registry Rust, frame Dart e coordinator Kotlin. Cancel e commit de criação disputam o mesmo `pending.remove`, e testes forçam as duas ordens. `vault.create/update/delete` e `locker.create` agora coalescem retries concorrentes e concluídos por escopo + request ID + HMAC do payload: o mesmo pedido reutiliza a resposta sob o binding da sessão atual, enquanto mudar operação/payload falha fechado. O cache limitado não retém payloads secretos nem respostas plaintext de leitura/unlock; testes provam commit/gesto único, retry em outro binding e conflito. |
...
| FLK-11 | P2 | ✅ | Explorer recebe verbos per-user de lock/unlock, ProgID sem sequestrar associação existente e atalho `.cmd` que aceita drag-and-drop; o uninstall remove somente as chaves/atalhos do PhoneAuth. Nautilus recebe scripts multi-seleção, MIME `.balock` e launcher `%F` para drag-and-drop via instalador reversível sob `$XDG_DATA_HOME`. Ambos chamam a CLI por arrays de argumentos, recusam seleção mista/não regular e preservam original/container; lock sempre escolhe destino do recovery. Testes nativos Linux/Windows entram no CI e o empacotamento Electron, tarball e Nix leva os launchers. Drive virtual/FUSE permanece explicitamente fora do MVP. |
...
| REL-08 | P1 | 🧪 | `deny.toml` separa advisories de licenças, admite só permissivas, bane `openssl` e registries fora do crates.io; os sete crates são `publish = false`. OSV varre os quatro lockfiles e o release guarda SBOM CycloneDX. O processo que faltava está em `docs/dependencies.md`: uma stack por PR, update nomeado e revisão de transitivas, teste no MSRV, pins Flutter sincronizados, fronteira Kotlin/AGP/Gradle/Java explícita, smoke do Electron empacotado, fixtures v1 imutáveis e rollback revertendo manifest+lock sem apagar dado nem reutilizar versão. Continua 🧪 somente até os jobs novos de supply chain e Windows passarem no CI pela primeira vez. |
...
| REL-12 | P2 | ✅ | Windows agora verifica automaticamente a release estável mais nova e baixa sem executar silenciosamente. O executável instalado é a raiz de confiança: antes de acessar a rede ele precisa de Authenticode válido e versão de arquivo igual à versão em execução. Release, URL, nome, tamanho e semver são canônicos; instalador novo e rollback precisam do mesmo **public key** do signer e da versão de arquivo esperada, conferidos de novo imediatamente antes de executar. Um update só é oferecido depois de preservar o instalador assinado da versão atual; a versão nova expõe rollback explícito no tray sem reescrever dados. Build dev, Linux e Windows sem assinatura não baixam nada. A pipeline agora entrega o certificado ao electron-builder antes do pacote e valida app, helpers e instalador depois. Rotação de chave é fail-closed e manual; procedimento, frequência, limites e recuperação estão em `docs/updates.md`. 7 testes de updater/workflow + verificação Windows negativa no CI. |
