...
/// A phone, reached over one already established session.
...
mod tests {
    ...
    fn a_wrap_asks_for_locker_create_and_returns_a_phone_wrapper() {
        ...
        let mut custodian = PhoneCustodian::new(&mut transport, "Workstation", "locker-cred-1");
        let dek = Dek::random();
    ...
    fn a_rekey_says_so_instead_of_looking_like_an_unlock() {
        ...
        let mut custodian =
            PhoneCustodian::new(&mut transport, "Workstation", "locker-cred-1").rekeying();
        let wrapper = Wrapper {
            kind: WrapperKind::Phone,
    ...
    fn an_unauthenticated_channel_never_sees_a_data_key() {
        let mut transport = session(false, |request| reply(request, Vec::new()));
        let mut custodian = PhoneCustodian::new(&mut transport, "Workstation", "locker-cred-1");
        let dek = Dek::random();
        assert!(matches!(
            custodian.wrap(&EngineWrapRequest {
    ...
    fn a_recovery_wrapper_is_never_sent_to_the_phone() {
        let mut transport = session(true, |request| reply(request, Vec::new()));
        let mut custodian = PhoneCustodian::new(&mut transport, "Workstation", "locker-cred-1");
        let wrapper = Wrapper {
