...
/// Number of equally likely words available to the passphrase generator.
...
/// Generates a passphrase from the EFF large wordlist.
///
/// Each word contributes `log2(7776)` bits of entropy. `separator` is copied
/// verbatim between words and does not affect that entropy.
pub fn generate_passphrase(word_count: usize, separator: &str) -> Zeroizing<String> {
    // Reserved up front, and this is not an optimisation.
    //
    // A `String` that grows copies its contents into a new allocation and
    // abandons the old one. `Zeroizing` wipes the buffer it is holding when it
    // drops — the last one — so every intermediate copy stays on the heap
    // untouched, each a prefix of the passphrase. Reserving the upper bound
    // means there is only ever one buffer, and that one is wiped.
    let mut passphrase = Zeroizing::new(String::with_capacity(passphrase_capacity(
        word_count, separator,
    )));
    for position in 0..word_count {
        if position != 0 {
            passphrase.push_str(separator);
        }
        let line = EFF_LARGE_WORDLIST
            .lines()
            .nth(index_below(EFF_WORD_COUNT))
            .expect("EFF wordlist must contain 7776 entries");
        let (_, word) = line
            .split_once('\t')
            .expect("EFF wordlist entries must be tab-separated");
        passphrase.push_str(word);
    }
    passphrase
}

/// The longest word in the EFF large wordlist, in bytes.
///
/// The list is ASCII, so bytes and characters agree. Asserted against the
/// embedded file in the tests, so a swapped wordlist fails there rather than
/// by quietly reintroducing a reallocation.
pub const EFF_LONGEST_WORD: usize = 9;

/// An upper bound on the bytes [`generate_passphrase`] will produce.
///
/// Exact enough that the buffer is never grown, which is the point: see the
/// comment in `generate_passphrase`.
pub fn passphrase_capacity(word_count: usize, separator: &str) -> usize {
    word_count * EFF_LONGEST_WORD + word_count.saturating_sub(1) * separator.len()
}

/// Entropy of a passphrase generated by [`generate_passphrase`], in bits.
pub fn passphrase_entropy_bits(word_count: usize) -> f64 {
    (EFF_WORD_COUNT as f64).log2() * word_count as f64
}

/// A uniform index in `0..bound`, by rejection.
///
/// Values at or above the largest multiple of `bound` are thrown away rather
/// than folded, which is the whole difference between this and `% bound`.
///
...
fn index_below(bound: usize) -> usize {
    assert!(bound != 0, "random index bound must not be zero");
...
mod tests {
    ...
    fn eff_words() -> Vec<&'static str> {
        EFF_LARGE_WORDLIST
            .lines()
            .map(|line| line.split_once('\t').expect("dice index and word").1)
            .collect()
    }

    #[test]
    fn the_eff_wordlist_is_complete_unique_and_ordered() {
        let words = eff_words();
    ...
    #[test]
    fn declared_passphrase_entropy_matches_the_wordlist_size() {
        for word_count in [0, 1, 6, 12] {
            assert_eq!(
                passphrase_entropy_bits(word_count),
                (EFF_WORD_COUNT as f64).log2() * word_count as f64
            );
        }
    }
    ...
    #[test]
    fn a_separator_does_not_change_the_passphrase_word_count() {
        let word_count = 6;
        let passphrase = generate_passphrase(word_count, "::");
        let words = eff_words();
        assert_eq!(passphrase.split("::").count(), word_count);
        assert!(passphrase.split("::").all(|word| words.contains(&word)));
    }
