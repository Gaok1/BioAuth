...
void main() {
  ...
  test('locker.create devolve um wrapper amarrado à credencial do telefone', () async {
  ...
  test('o telefone usa a credencial que ele guarda, não a que veio no frame', () async {
    final other = LockerService(guardian: guardian, credentialId: 'outra-cred');
    await other.handle(
      _request(lockerUnlockOperation, _unwrapPayload()),
      sessionBinding: _sessionBinding,
      now: _now,
  ...
  test('uma operação de outro produto não é atendida', () async {
    ...
    expect(
      ApplicationFrame.decode(answer).kind,
