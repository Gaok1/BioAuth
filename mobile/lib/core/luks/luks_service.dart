...
class LuksService {
  LuksService({
    required LuksKeyGuardian guardian,
    required String credentialId,
    ApplicationIdempotency? retries,
  }) : _guardian = guardian,
       _credentialId = credentialId,
       // Passed in, never owned. This service is built to answer one frame and
       // dropped, so a cache it allocates for itself is empty every time it is
       // read -- a retry cache that has never seen a retry. The session service
       // holds the one that outlives the connection.
    ...
    try {
      final outcome = request.operation == luksEnrollOperation
          ? await _idempotency.run(
              // The desktop picks the credential id, so two of them can
              // pick the same one. Sharing a cache across desktops makes that
              // a collision instead of a coincidence -- and the answer being
              // handed to the wrong one here is a wrapped disk key.
